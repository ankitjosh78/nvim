# nvim
 My vimrc (init.vim)

# Screenshot
  ## Nerd Tree
  ![Nerd Tree](https://github.com/ankitjosh78/nvim/blob/main/ss/nerd_tree.png?raw=true)
  ## Find Files
  ![Find Files](https://github.com/ankitjosh78/nvim/blob/main/ss/find_files.png?raw=true)
  ## Live Grep
  ![Live Grep](https://github.com/ankitjosh78/nvim/blob/main/ss/live_grep.png?raw=true)
  ## Document Symbols
  ![Document Symbols](https://github.com/ankitjosh78/nvim/blob/main/ss/document_symbols.png?raw=true)
  ## Current Buffer Fuzzy Find
  ![Current Buffer Fuzzy Find](https://github.com/ankitjosh78/nvim/blob/main/ss/current_buffer_fuzzy.png?raw=true)
